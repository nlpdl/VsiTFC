...
@dataclass
class old_CriterionConfig(FairseqDataclass):
...
def construct_target(targets, target_scores, num_tokens):
        """
        Args:
...
@register_criterion(
    "old_criterion", dataclass=old_CriterionConfig
)
class OldCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
        ...
        #        weight=1.
        #     )
        # self.linear_bca_past = nn.Linear(128, 256)
        # self.criterion.add(
        #     "bca_past",
        #     MSELoss(),
        #     weight=1.
        # )
        # self.linear_bca_future = nn.Linear(128, 256)
        # self.criterion.add(
        #     "bca_future",
        #     MSELoss(),
        #     weight=1.
    ...
    def forward(self, model, sample, reduce=True):
        ...
        main_loss, nll_loss = label_smoothed_nll_loss(
            lprobs,
            target,
            self.eps,
        ...
        future_capsule = net_output[1]['future_caps']

        future_info = net_output[1]['future_info_emb']

        print('future_info',future_info.shape)

        scores = get_average_score(future_info)

        scores = bottle(scores)

        future_capsule = bottle(future_capsule)

        future_info = bottle(future_info)

        num_tokens = future_capsule.size(-1)
        gtruth = construct_target(future_info, scores, num_tokens)

        l1loss = self.l1loss(future_capsule,gtruth)
        ...
        # past_caps_lprobs = model.get_normalized_probs(tuple([net_output[1]['past_caps']]), log_probs=True)
        # params_nmt = dict(
        #     inputs=lprobs,
        #     labels=target,
        #     update=True)
        ...
        # if target is not None: future_info_mask = (target != 0).float()
        # future_info_mask = Variable(future_info_mask, requires_grad=False)
        # oov_loss_list = []
        # for di in range(target.size(1)):
        #     oov_loss= self.my_criterion(
        #         lprobs[:,di],
        #         target[:,di],
        #     )
        #     oov_loss_list += [oov_loss * future_info_mask[:, di]]
        # for i, m in enumerate(oov_loss_list):
        #     oov_loss_list[i] = oov_loss_list[i][:, None]
        # oov_loss_list = torch.cat(oov_loss_list, dim=1) # bsz, seq_len
        # loss_mt_mean = oov_loss_list.view(1, -1).sum(-1) / future_info_mask.view(1, -1).sum(-1)
        # params_dict = {}


        past_labels, past_scores = convert_to_past_labels(net_output[1]['prev_output_tokens'])
            
        # params_wploss_past = dict(
        #     inputs=past_caps_lprobs,
        #     labels=past_labels,
        ...
        # past_nonpad_mask, future_nonpad_mask = net_output[1]['prev_output_tokens'].ne(1), target.ne(1)
        # params_bca_past = dict(
        #     inputs=net_output[1]['past_caps'],
        #     labels=get_prev_sequence_average(net_output[0]),
        #     mask=past_nonpad_mask,
        #     update=True
        # )
        # params_bca_future = dict(
        #     inputs=net_output[1]['future_caps'],
        #     labels=get_sub_sequence_average(net_output[0]),
        ...
        lambd = 1
        logging_output = {
            "loss": main_loss.data+lambd*l1loss.data,
            "nll_loss": nll_loss,
        ...
        #     print('target',target)
        #     assert 1 == 0
        return main_loss+l1loss, sample_size, logging_output

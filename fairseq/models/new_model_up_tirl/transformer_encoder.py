...
from fairseq.models.new_model_up_tirl import transformer_layer
...
from fairseq.models.new_model_up_tirl.utils import SublayerConnectionv2,PositionwiseFeedForward
...
class TransformerEncoderBase(FairseqEncoder):
    ...
    def __init__(self, cfg, dictionary, embed_tokens, return_fc=False):
        ...
        self.x_attn = self.build_self_attention(256,cfg)
    ...
    def forward(
        ...
        return self.forward_scriptable(
            src_tokens, src_lengths, return_all_hiddens, token_embeddings,img_global,matrix = matrix
        )
    ...
    def forward_scriptable(
        ...
        matrix_o = matrix.transpose(1,2)# b * o * t
        batch, objn, xn = matrix_o.size(0), matrix_o.size(1), matrix_o.size(2)
        ...
        for id ,layer in enumerate(self.layers):
            lr = layer(
            x,x, encoder_padding_mask=encoder_padding_mask if has_pads else None
            )  
        ...
        # #b * o * d
        # o_x = (o_x_gate * matrix_o * x_ep).sum(2)
        ...
        return {
            "encoder_out": [final_x.transpose(0,1)],  # T x B x C

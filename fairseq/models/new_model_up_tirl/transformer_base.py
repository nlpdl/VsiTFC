...
from fairseq.models.new_model_up_tirl import (
    TransformerConfig,
...
class TransformerModelBase(FairseqEncoderDecoderModel):
    ...
    def forward(
        ...
        word_emb_list = decoder_out[1]['future_caps']
        word_emb_h = decoder_out[1]["no_gate_x"]
        # word_emb = self.relu(self.L2_linear(word_emb))
        sim_list = []
        sim2_list = []
        for id,word_emb in enumerate(word_emb_list):
            sim = calculate_cosine_distance_step(word_emb,img_global_emb,fr_o,id + 1)
            # if sim.data < 0:
            #     sim.data = torch.tensor(0).cuda()
            
            sim2 = calculate_cosine_distance_step(word_emb_h,img_global_emb,fr_o,id + 1)
            # if sim2.data < 0:
        ...
        future_caps_out_list = []
        for item in decoder_out[1]['future_caps']:
            future_caps_out_list.append(self.output_layer(item))




        decoder_out[1]['future_caps'] = future_caps_out_list
        ...
        decoder_out[1]['sim'] = sum(sim_list)
        decoder_out[1]['sim2'] = sum(sim2_list)

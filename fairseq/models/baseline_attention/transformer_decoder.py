...
from fairseq.models.baseline_model.utils import *
import torch
...
from fairseq.models.baseline_attention import TransformerConfig
...
class TransformerDecoderBase(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        capsule_query = self.pre_capsule_layer_norm(oov)
        ...
        # assert 1 == 0
        # output = self.out_and_cap_ffn(x, future_caps)

        if self.layer_norm is not None:
            output = self.layer_norm(output)

        if self.project_out_dim is not None:
            output = self.project_out_dim(output)
        

        return x, {"attn": [attn], "inner_states": inner_states,"past_caps":past_caps,"future_caps":future_caps}

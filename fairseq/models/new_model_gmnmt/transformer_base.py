...
from fairseq.models.new_model_gmnmt import (
    TransformerConfig,
...
class TransformerModelBase(FairseqEncoderDecoderModel):
    ...
    def __init__(self, cfg, encoder, decoder):
        ...
        # self.future_layer_norm = torch.nn.LayerNorm(256, 1e-5, True)
        # self.img_layer_norm = torch.nn.LayerNorm(256, 1e-5, True)
    ...
    def forward(
        ...
        encoder_out = self.encoder(
            src_tokens, src_lengths=src_lengths, return_all_hiddens=return_all_hiddens,img_global = img_global,graph = graph,matrix = matrix,
        )
        ...
        img_global_emb = self.img_linear(img_global)#转换一下形状
        limit_index = encoder_out['limit_index']
        word_emb_h = decoder_out[1]["no_gate_x"]
        sim2 = calculate_cosine_distance(word_emb_h,img_global_emb,fr_o)
        # sim2 = calculate_cosine_distance_limit(word_emb_h,img_global_emb,fr_o,limit_index)
        ...
        # sim = calculate_cosine_distance_limit(word_emb,img_global_emb,fr_o,limit_index)
        # if sim.data < 0:

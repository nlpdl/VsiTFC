...
from fairseq.models.new_model_gmnmt import TransformerConfig
...
class TransformerDecoderBase(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        final_x = encoder_out["encoder_out"][0].transpose(0,1)
        # embed positions
        positions = None
        ...
        return x, {"attn": [attn], "inner_states": inner_states,"x":x,"future_caps":future_caps,"now_x":now_x,"no_gate_x":no_gate_x,"img_global":img_global}
...
class GateModel(nn.Module):
    def __init__(self,embed_dim):
        super().__init__()
        # self.newmodel = newModel(embed_dim)
...
class newModel(nn.Module):
    def __init__(self,embed_dim):
        ...
        self.layer_norm = torch.nn.LayerNorm(embed_dim, 1e-5, True)
    def forward(self,futrue):
        futrue = self.layer_norm(self.linear2(torch.tanh(self.linear1(futrue))))
        return futrue

...
from fairseq.models.new_model_gmnmt.utils import *


class TranformerEncoderLayerFusion(nn.Module):
    def __init__(self, cfg, return_fc=False):
        ...
        self.self_x_attn = self.build_self_attention(self.embed_dim, cfg)
        self.ffn_x = PositionwiseFeedForward(self.embed_dim,cfg.encoder_ffn_embed_dim)
        self.res4ffn_x = SublayerConnectionv2(self.embed_dim, self.dropout)
        self.res4mes_x = SublayerConnectionv2(self.embed_dim, self.dropout)

        self.self_o_attn = self.build_self_attention(self.embed_dim, cfg)
        self.ffn_o = PositionwiseFeedForward(self.embed_dim,cfg.encoder_ffn_embed_dim)
        self.res4ffn_o = SublayerConnectionv2(self.embed_dim, self.dropout)
        self.res4mes_o = SublayerConnectionv2(self.embed_dim, self.dropout)
        self.norm = nn.LayerNorm(self.embed_dim)

        # self.mhatt_x2o1 = nn.Linear(self.embed_dim, self.embed_dim)
        # self.mhatt_x2o2 = nn.Linear(self.embed_dim, self.embed_dim)
        # self.mhatt_o2x1 = nn.Linear(self.embed_dim, self.embed_dim)
        # self.mhatt_o2x2 = nn.Linear(self.embed_dim, self.embed_dim)
        self.mhatt_x2o = nn.Linear(self.embed_dim * 2, self.embed_dim)
        self.mhatt_o2x = nn.Linear(self.embed_dim * 2, self.embed_dim)
        self.xgate = SublayerConnectionv2(self.embed_dim, cfg.dropout)
        self.ogate = SublayerConnectionv2(self.embed_dim, cfg.dropout)
    ...
    def forward(        
        ...
        newx = self.res4mes_x(res_x, x)


        # Visual Self-attention, for image node
        # res_o = o
        # o, _ = self.self_x_attn(
        #     query=o,
        #     key=o,
        #     value=o,
        # )
        
        # newo = self.res4mes_o(res_o, o)


        newo = self.norm(o)
        newx = newx.transpose(0,1)
        newo = newo.transpose(0,1)
        # newo = o.transpose(0,1)
        ...
        # B T O H
        # 15 * 10 * 256
        # x2o_gates = torch.sigmoid(self.mhatt_x2o1(newx_ep) + self.mhatt_x2o2(o_ep))
        x2o_gates = torch.sigmoid(self.mhatt_x2o(torch.cat((newx_ep, o_ep), -1)))

        x2o = (x2o_gates * matrix * o_ep).sum(2)
        x2o = (x2o_gates * matrix * newx_ep).sum(2)
        ...
        # B O T H
        # o2x_gates = torch.sigmoid(self.mhatt_x2o1(x_ep) + self.mhatt_x2o2(newo_ep))
        o2x_gates = torch.sigmoid(self.mhatt_o2x(torch.cat((x_ep, newo_ep), -1)))
        o2x = (o2x_gates * matrix4obj * x_ep).sum(2)


        newx = self.xgate(newx, x2o)
        newo = self.ogate(newo, o2x)

        # using ffn to update
        x = self.res4ffn_x(newx, self.ffn_x(newx))
        o = self.res4ffn_o(newo, self.ffn_o(newo))

...
from fairseq.models.new_model_gru import TransformerConfig
...
class TransformerDecoderBase(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        img_global = img_global.transpose(0,1)# l * b * h -> b * l * h
        now_x = x#记录当前x
        start_x = x
        ...
        # img_global = img_global.transpose(0,1)# b * l * h -> l * b * h


        temp_o = o.sum(0).unsqueeze(0)

        print('o',o.shape)

        future_caps,_ = self.gru(capsule_query.transpose(0,1),hx=o)

        assert 1 == 0


        img_global = img_global.transpose(0,1)# l * b * h -> b * l * h
        future_caps = future_caps.transpose(0,1)
        ...
        return x, {"attn": [attn], "inner_states": inner_states,"x":x,"future_caps":future_caps,"now_x":now_x,"no_gate_x":no_gate_x}
